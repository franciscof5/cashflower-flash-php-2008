<?php
session_start();
#<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd" lang="pt_BR">
...
if($email!=NULL and $senha!=NULL) {
	...
	$queryEmail = mysqli_query($conexao, $queryEmailString) or die(mysqli_error($conexao));
	...
	if($queryEmailArray["email"]!=$email) {
		...
		$querySenha = mysqli_query($conexao, $querySenhaString) or die(mysqli_error($conexao));
		...
		if($querySenhaArray["senha"]==$senha) {
			...
			$queryContaID = mysqli_query($querySenhaString, $conexao) or die(mysqli_error($conexao));
			$queryContaIDArray = mysqli_fetch_assoc($querySenha);
			...
			$queryUserID = mysqli_query($conexao, $queryUserIDString) or die(mysqli_error($conexao));
			$queryUserIDArray = mysqli_fetch_assoc($queryUserID);
			...
			$queryContaString = 'SELECT `userID`,`contaBancariaID` FROM `relusercont` WHERE `userID` = "'.$queryUserIDArray["userID"].'";';
			$queryConta = mysqli_query($conexao, $queryContaString) or die(mysqli_error($conexao));
			$queryContaArray = mysqli_fetch_assoc($queryConta);
			#var_dump($queryContaArray);die;
			...
			if(isset($queryContaArray["userID"])) {
				...
				$_SESSION["contaBancariaID"] = $queryContaArray["contaBancariaID"];
						...
						<table cellspacing="5" cellpadding="10">
							<tr valign="top">
								<td width="290" rowspan="2">
									...
									<p>É um novo sistema	de fluxo de caixa online, desenvolvido por <a href="http://www.franciscomat.com" target="_blank">Francisco Mat</a>.</p>
